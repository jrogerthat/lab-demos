async function loadData(file, type) {
  ...
  let mapPath =
    "https://gist.githubusercontent.com/jrogerthat/13d202baa32e3f40935fd821a53a2473/raw/5b6e885727ccc290d0f62fa9c5fa69e81db6cf37/geojson-us.json";
  let ufoPath =
    "https://gist.githubusercontent.com/jrogerthat/d268681ca1aa08f1550b8fd128865eef/raw/af4dfd3b11040dc3f4296c9138b562445e969841/ufo-sample-data";
  let stateDictPath =
  ...
  loadData(mapPath, "json").then(async mapData => {
    ...
    console.log(stateDict[state.properties.name]);
  ...
  function drawMap(svg, projection, mapData) {
   ...
   // console.log(mapData.features.map(m => m.properties.ufos.length));
    ...
    let min = d3.min(mapData.features.map(m => {
        
        m.properties.ufos.length}));
