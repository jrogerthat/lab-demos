let filePath = "https://raw.githubusercontent.com/lhcb/opendata-project/master/Data/nobel.csv";

d3.csv(filePath).then((d,i)=> {
  let data = d.filter(f=> (f['Residence'] != 'No Data') && (f['Role/Affiliate'] != 'No Data')).slice(0, 20);
  console.log('data',data);
  let columnKeys = d3.keys(data[0]).map(k=> {return {'head': k, 'sorted':false}});
...
function drawTable(bodyData, headerData){
  ...
  let headers = table.select('thead')
        .selectAll('th')
        .data(headerData).join('th')
        .text(d=> d.head);

  // create table body
  let tableRows = table.select('tbody').selectAll('tr').data(bodyData).join('tr');

  let cells = tableRows.selectAll('td').data(d => {
    console.log('before using d3.entries: ',d);
    console.log('after using d3.entries: ', d3.entries(d))
    return d3.entries(d)}).join('td');
